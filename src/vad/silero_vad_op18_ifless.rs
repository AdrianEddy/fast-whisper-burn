...
#[derive(Module, Debug)]
pub struct Model {
    constant32: burn::module::Param<Tensor<1, Int>>,
    constant41: burn::module::Param<Tensor<1, Int>>,
...
impl Model {
    ...
    pub fn forward_sequence_16khz(
        ...
        let unsqueeze31_out1: Tensor<3> = pad7_out1.unsqueeze_dims::<3>(&[1]);
        ...
        let hidden: Tensor<3> = hidden.unsqueeze_dims::<3>(&[0]);
        let cell: Tensor<3> = cell.unsqueeze_dims::<3>(&[0]);
...
impl Model {
    ...
    pub fn new(device: &Device) -> Self {
        let constant32: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<1, Int>::from_data([0i64], device),
            device.clone(),
            false,
            [1].into(),
        );
        let constant41: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<1, Int>::from_data([1i64], device),
            device.clone(),
            false,
            [1].into(),
        );
        let constant42: burn::module::Param<Tensor<1>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<1>::from_data([2f64], device),
            device.clone(),
            false,
            [1].into(),
        );
    ...
    pub fn forward(
        ...
        let (if1_out1, if1_out2) = if equal1_out1 {
            ...
            let unsqueeze32_out1: Tensor<3> = mul21_out1.clone().unsqueeze_dims::<3>(&[-1]);
            let unsqueeze33_out1: Tensor<3> = mul21_out1.unsqueeze_dims::<3>(&[0]);
            let unsqueeze34_out1: Tensor<3> = add21_out1.unsqueeze_dims::<3>(&[0]);
            let concat7_out1 =
            ...
            let unsqueeze35_out1: Tensor<2> = reducemean7_out1.unsqueeze_dims::<2>(&[1]);
            ...
            let unsqueeze36_out1: Tensor<3> = pad8_out1.unsqueeze_dims::<3>(&[1]);
            ...
            let unsqueeze37_out1: Tensor<3> = mul24_out1.clone().unsqueeze_dims::<3>(&[-1]);
            let unsqueeze38_out1: Tensor<3> = mul24_out1.unsqueeze_dims::<3>(&[0]);
            let unsqueeze39_out1: Tensor<3> = add24_out1.unsqueeze_dims::<3>(&[0]);
            let concat8_out1 =
            ...
            let unsqueeze40_out1: Tensor<2> = reducemean8_out1.unsqueeze_dims::<2>(&[1]);
